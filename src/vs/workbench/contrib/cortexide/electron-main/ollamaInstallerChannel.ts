...
export class OllamaInstallerChannel implements IServerChannel {
  ...
  private install(params: InstallParams) {
    ...
    if (isMac) {
      ...
      const script = [
        'set -e',
        'echo [CortexIDE] macOS install starting...',
        'if [ -d /Applications/Ollama.app ]; then echo [CortexIDE] Found /Applications/Ollama.app; open -a Ollama; else',
        ...
        '   eval "$([ -x /opt/homebrew/bin/brew ] && /opt/homebrew/bin/brew shellenv || /usr/local/bin/brew shellenv)";',
        ' fi;',
        ' echo [CortexIDE] Installing Ollama via Homebrew Cask...; brew install --cask ollama || true; open -a Ollama; fi',
        'sleep 2',
        'echo [CortexIDE] Health check...',
        'curl -fsS http://127.0.0.1:11434/api/tags >/dev/null 2>&1 && echo [CortexIDE] Ollama running || echo [CortexIDE] Ollama not reachable yet'
      ].join('\n');
    ...
    if (isLinux) {
      ...
      const script = [
        'set -e',
        'echo [CortexIDE] Linux install starting...',
    ...
    const ps = [
      ...
      'Write-Host "[CortexIDE] Windows install starting...";',
      ...
      'try { $r = Invoke-WebRequest -UseBasicParsing http://127.0.0.1:11434/api/tags -TimeoutSec 5; if ($r.StatusCode -eq 200) { Write-Host "[CortexIDE] Ollama running" } } catch { Write-Host "[CortexIDE] Ollama not reachable yet" }'
    ].join('\n');
