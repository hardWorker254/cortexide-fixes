...
import { ICortexideSCMService } from '../common/cortexideSCMTypes.js'
...
class GenerateCommitMessageService extends Disposable implements IGenerateCommitMessageService {
	...
	private voidSCM: ICortexideSCMService
	...
	constructor(
		...
		this.voidSCM = ProxyChannel.toService<ICortexideSCMService>(mainProcessService.getChannel('cortexide-channel-scm'))
	}
	...
	async generateCommitMessage() {
		...
		this.execute.trigger(async () => {
			...
			try {
				...
				const modelSelection = this.cortexideSettingsService.state.modelSelectionOfFeature['SCM'] ?? null
				...
				const modelSelectionOptions = modelSelection && !(modelSelection.providerName === 'auto' && modelSelection.modelName === 'auto')
					? this.cortexideSettingsService.state.optionsOfModelSelection['SCM'][modelSelection.providerName]?.[modelSelection.modelName]
					: undefined
				const overridesOfModel = this.cortexideSettingsService.state.overridesOfModel
...
class GenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			...
			title: localize2('voidCommitMessagePrompt', 'CortexIDE: Generate Commit Message'),
			icon: ThemeIcon.fromId('sparkle'),
			tooltip: localize2('voidCommitMessagePromptTooltip', 'CortexIDE: Generate Commit Message'),
			f1: true,
			menu: [{
...
class LoadingGenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			...
			title: localize2('voidCommitMessagePromptCancel', 'CortexIDE: Cancel Commit Message Generation'),
			icon: ThemeIcon.fromId('stop-circle'),
			tooltip: localize2('voidCommitMessagePromptCancelTooltip', 'CortexIDE: Cancel Commit Message Generation'),
