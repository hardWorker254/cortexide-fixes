...
export type CortexideSelectionHelperProps = {
...
export class SelectionHelperContribution extends Disposable implements IEditorContribution, IOverlayWidget {
	...
	constructor(
		...
		@ICortexideSettingsService private readonly _cortexideSettingsService: ICortexideSettingsService
	) {
		super();
	...
	private _showHelperForSelection(selection: Selection): void {
		...
		// rerender
		const enabled = this._cortexideSettingsService.state.globalSettings.showInlineSuggestions
		...
		if (enabled) {
			this._rerender({ rerenderKey: this._rerenderKey } satisfies CortexideSelectionHelperProps)
