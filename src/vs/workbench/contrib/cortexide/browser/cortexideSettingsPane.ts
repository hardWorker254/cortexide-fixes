...
class CortexideSettingsInput extends EditorInput {
	...
	readonly resource = CortexideSettingsInput.RESOURCE;
	...
	override get typeId(): string {
		return CortexideSettingsInput.ID;
	}

		override getName(): string {
			return nls.localize('voidSettingsInputsName', 'CortexIDE Settings');
		}
...
class CortexideSettingsPane extends EditorPane {
	...
	constructor(
		...
		super(CortexideSettingsPane.ID, group, telemetryService, themeService, storageService);
	}
...
Registry.as<IEditorPaneRegistry>(EditorExtensions.EditorPane).registerEditorPane(
	EditorPaneDescriptor.create(CortexideSettingsPane, CortexideSettingsPane.ID, nls.localize('CortexideSettingsPane', "CortexIDE Settings Pane")),
	[new SyncDescriptor(CortexideSettingsInput)]
);
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: CORTEXIDE_TOGGLE_SETTINGS_ACTION_ID,
			title: nls.localize2('voidSettings', "CortexIDE: Toggle Settings"),
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		// if is open, close it
		const openEditors = editorService.findEditors(CortexideSettingsInput.RESOURCE); // should only have 0 or 1 elements...
		...
		// else open it
		const input = instantiationService.createInstance(CortexideSettingsInput);
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: CORTEXIDE_OPEN_SETTINGS_ACTION_ID,
			title: nls.localize2('voidSettingsAction2', "CortexIDE: Open Settings"),
			f1: true,
			icon: Codicon.settingsGear,
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const openEditors = editorService.findEditors(CortexideSettingsInput.RESOURCE);
		if (openEditors.length > 0) {
			await editorService.closeEditors(openEditors);
		}
...
MenuRegistry.appendMenuItem(MenuId.GlobalActivity, {
	...
	command: {
		id: CORTEXIDE_TOGGLE_SETTINGS_ACTION_ID,
		title: nls.localize('voidSettingsActionGear', "CortexIDE Settings")
	},
	order: 1
});
