...
import { isFeatureNameDisabled } from '../../../../common/cortexideSettingsTypes.js'
...
export const useEditToolStreamState = ({ applyBoxId, uri }: { applyBoxId: string, uri: URI }) => {
	const accessor = useAccessor()
	const cortexideCommandBarService = accessor.get('ICortexideCommandBarService')
	const [streamState, setStreamState] = useState(cortexideCommandBarService.getStreamState(uri))
	// listen for stream updates on this box
	useCommandBarURIListener(useCallback((uri_) => {
		const shouldUpdate = uri.fsPath === uri_.fsPath
		if (shouldUpdate) { setStreamState(cortexideCommandBarService.getStreamState(uri)) }
...
const ApplyButtonsForEdit = ({
	...
	const onClickSubmit = useCallback(async () => {
		...
		if (!applyDonePromise) {
				notificationService.info(`CortexIDE Error: We couldn't run Apply here. ${uri === 'current' ? 'This Apply block wants to run on the current file, but you might not have a file open.' : `This Apply block wants to run on ${uri.fsPath}, but it might not exist.`}`)
		}
		...
		applyDonePromise?.catch(e => {
			...
			notificationService.info(`CortexIDE Error: There was a problem running Apply: ${e}.`)
