...
import { isValidProviderModelSelection } from './cortexideSettingsTypes.js';
	...
	async parseNLToShell(nlInput: string, cwd: string | null, token: CancellationToken): Promise<ParsedShellCommand> {
		...
		if (modelSelection.providerName === 'auto' && modelSelection.modelName === 'auto') {
			...
			if (fallbackModel) {
				...
				throw new Error('No model provider configured. Please configure a model provider in CortexIDE Settings.');
			}
		}
