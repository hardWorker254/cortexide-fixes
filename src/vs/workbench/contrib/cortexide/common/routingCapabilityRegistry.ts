...
export interface ModelCapabilityProfile {
	...
	rawCapabilities: CortexideStaticModelInfo;
}
...
export class ModelCapabilityRegistry {
	...
	private computeProfile(
		modelSelection: ModelSelection,
		capabilities: CortexideStaticModelInfo
	): ModelCapabilityProfile {
	...
	private computeCodeEditStrength(provider: string, name: string, capabilities: CortexideStaticModelInfo): number {
		let score = 0;
	...
	private computeCodeReasoningStrength(provider: string, name: string, capabilities: CortexideStaticModelInfo): number {
		let score = 0;
		...
		if (provider === 'anthropic' && (name.includes('4') || name.includes('opus') || name.includes('3.5') || name.includes('sonnet'))) {
			score += 0.25;
		} else if (provider === 'openai' && (name.includes('4o') || name.includes('4.1') || name.includes('gpt-4'))) {
			score += 0.25;
		}
	...
	private computeGeneralQAStrength(provider: string, name: string, capabilities: CortexideStaticModelInfo): number {
	...
	private computeVisionStrength(provider: string, name: string, capabilities: CortexideStaticModelInfo): number {
		...
		if ((provider === 'ollama' || provider === 'vllm') && (name.includes('llava') || name.includes('bakllava') || name.includes('vision'))) {
			return 0.6; // Local vision models are weaker
		}
		return 0.0;
	}

	private computePDFStrength(provider: string, name: string, capabilities: CortexideStaticModelInfo): number {
	...
	private computeLongContextStrength(capabilities: CortexideStaticModelInfo): number {
