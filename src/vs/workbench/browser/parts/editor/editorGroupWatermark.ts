...
/* eslint-disable */ // Void
import { CORTEXIDE_CTRL_K_ACTION_ID, CORTEXIDE_CTRL_L_ACTION_ID } from '../../../contrib/cortexide/browser/actionIDs.js';
...
export class EditorGroupWatermark extends Disposable {
	...
	constructor(
		...
		const elements = h('.editor-group-watermark', [
			h('.letterpress.void-void-icon@icon'),
		...
		const logoUri = FileAccess.asBrowserUri('vs/workbench/browser/media/cortexide-main.png');
	...
	private render(): void {
		...
		const update = async () => {
			...
			if (this.contextService.getWorkbenchState() === WorkbenchState.EMPTY) {
				...
				// 	label3.set(keys3);
				// button3.onclick = () => {
				// 	this.commandService.executeCommand(CORTEXIDE_OPEN_SETTINGS_ACTION_ID)
				// }
